...
# Prepare data splits (same as Phase 3b)
data_splits = {}
...
for country in countries:
    ...
    checkpoint = torch.load(f'phase3b_lstm_results/lstm_model_{country}.pt', weights_only=False)
...
for country in countries:
    ...
    for i in range(n_windows):
        # Forecast origin
        forecast_start_idx = dev_end = int(0.9 * len(full_data))
        forecast_origin = forecast_start_idx + (i * 24)
...
for country in countries:
    lstm_mase = metrics_summary[country]['MASE']
    sarima_mase = sarima_metrics[country]['MASE']
    winner = "LSTM" if lstm_mase < sarima_mase else "SARIMA"
    print(f"  {country}: LSTM={lstm_mase:.4f} | SARIMA={sarima_mase:.4f} | Winner: {winner}")

print("\nsMAPE Comparison (lower is better):")
for country in countries:
    lstm_smape = metrics_summary[country]['sMAPE_%']
    sarima_smape = sarima_metrics[country]['sMAPE']
    winner = "LSTM" if lstm_smape < sarima_smape else "SARIMA"
    print(f"  {country}: LSTM={lstm_smape:.2f}% | SARIMA={sarima_smape:.2f}% | Winner: {winner}")

print("\nRMSE Comparison (lower is better):")
for country in countries:
    lstm_rmse = metrics_summary[country]['RMSE_MW']
    sarima_rmse = sarima_metrics[country]['RMSE']
    winner = "LSTM" if lstm_rmse < sarima_rmse else "SARIMA"
    print(f"  {country}: LSTM={lstm_rmse:.2f} | SARIMA={sarima_rmse:.2f} | Winner: {winner}")
