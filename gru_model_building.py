...
# Prepare data splits (same 120-day window as SARIMA)
data_splits = {}
for country, load_col in load_columns.items():
    data = df[load_col].dropna()
    
    # Use last 120 days (same as SARIMA Phase 3)
...
class GRUForecaster(nn.Module):
    """
    GRU (Gated Recurrent Unit) model for multi-step time series forecasting
    ...
    def __init__(self, input_size=1, hidden_size=128, num_layers=2, output_size=24, dropout=0.2):
        ...
        # GRU layers
        # GRU has reset gate and update gate (simpler than LSTM)
        self.gru = nn.GRU(
...
for country in countries:
    ...
    # Save model
    model_path = f"results/phase3d_gru_results/{country}_gru_model.pt"
    torch.save({
...
with open('results/phase3d_gru_results/training_summary.json', 'w') as f:
...
plt.tight_layout()
plt.savefig('results/phase3d_gru_results/training_curves.png', dpi=150, bbox_inches='tight')
plt.close()
...
summary_df.to_csv('results/phase3d_gru_results/training_summary.csv', index=False)
...
print("\nNext step: Run phase4d_gru_forecasting.py for rolling window forecasting")
